...
async function fetchSuggestions(rawWord: string, imeCode: string): Promise<string[]> {
  try {
    const url = `https://inputtools.google.com/request?text=${encodeURIComponent(rawWord)}&itc=${imeCode}&num=5&cp=0&cs=1&ie=utf-8&oe=utf-8&app=demopage`;
...
// ── PDF.js loader ──────────────────────────────────────────────────────────────
// FIX: pdfjs-dist's Node build tries to require('canvas') which doesn't exist
// in the browser bundle. We lazily import it only on the client side, and
// alias canvas → false in next.config.js (see instructions below).

type PdfjsLib = typeof import("pdfjs-dist");
const pdfjsCache: { lib: PdfjsLib | null } = { lib: null };

async function getPdfjsLib(): Promise<PdfjsLib> {
  if (typeof window === "undefined") {
    throw new Error("getPdfjsLib must only be called client-side");
  }
  if (pdfjsCache.lib) return pdfjsCache.lib;
  const pdfjs = await import("pdfjs-dist");
  pdfjs.GlobalWorkerOptions.workerSrc = "/js/pdf.worker.js";
  pdfjsCache.lib = pdfjs;
  return pdfjsCache.lib;
}
...
interface EditDocumentModalProps {
  ...
  onSave?: (updatedData: { file_path: string; ocr_data: OcrData }) => void;
}
...
export function EditDocumentModal({
  ...
  const [visibleLevels, setVisibleLevels] = useState<Set<Level>>(new Set<Level>(["word"]));
  ...
  const applyZoom = useCallback((next: number) => {
    ...
    stage.x(0); stage.y(0);
    stage.scaleX(clamped); stage.scaleY(clamped);
    const scaledW = pixelW * clamped;
    const scaledH = pixelH * clamped;
    stage.width(scaledW); stage.height(scaledH);
  ...
  const handleZoom = useCallback((direction: "in" | "out" | "reset") => {
    ...
    applyZoom(
      ...
      direction === "out" ? current - ZOOM_STEP : 1
    );
  }, [applyZoom]);
  ...
  useEffect(() => {
    const imeCode = LANG_IME_MAP[resolvedLang];
      ...
      if ([" ", ",", ".", "!", "?", ";", ":", "(", ")", "\n"].includes(e.key)) {
        setSuggestions((prev) => {
          if (prev && prev.words.length > 0) {
            ...
            const ta = tooltipInputRef.current;
            if (ta) {
              const before   = ta.value.slice(0, prev.caretPos - prev.rawWord.length);
              const after    = ta.value.slice(prev.caretPos);
  ...
  }, [tooltip?.id, resolvedLang, translitEnabled, acceptSuggestion]);
  ...
  // ── Fetch OCR ──────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!isOpen || !resolvedOcrUrl) return;
    setLoading(true);
    setError(null);
    (async () => {
      try {
        const res = await fetch(resolvedOcrUrl);
  ...
  }, [isOpen, deleteSelected, closeTooltip]);

  // ── Attach Handlers ────────────────────────────────────────────────────────
  const attachHandlers = useCallback((rect: Konva.Rect) => {
    rect.on("dragend",      () => syncRect(rect, rect.hasName("word") ? "word" : "line"));
    rect.on("transformend", () => syncRect(rect, rect.hasName("word") ? "word" : "line"));
    rect.on("click tap", (e) => {
      e.cancelBubble = true;
      select(rect.id());
  ...
  const buildCanvas = useCallback((
    ...
    stage.add(bgLayer);
    bgLayer.add(new Konva.Image({ image: bgSource, x: 0, y: 0, width: containerW, height: canvasH }));
    bgLayer.draw();
    ...
    stage.on("wheel", (e) => {
      ...
      if (evt.ctrlKey || evt.metaKey) {
        ...
        const clampedX = Math.min(0, pointer.x - contentX * next);
        const clampedY = Math.min(0, pointer.y - contentY * next);
  ...
  const buildForCurrentPage = useCallback(async (ocr: OcrData, page: number) => {
    ...
    if (isActuallyPdf) {
      ...
      try {
        ...
        // FIX: re-use the already-loaded pdfjs instance instead of re-importing
        const pdfjs = await getPdfjsLib();
      ...
      img.onload = () => {
        ...
        buildCanvas(ocr, pageKey, img, ocrW, ocrH, img.naturalWidth, img.naturalHeight);
      };
      img.onerror = () => setError("Failed to load image");
    }
  }, [imageUrl, isActuallyPdf, buildCanvas]);

  // ── Mount canvas when OCR is ready ────────────────────────────────────────
  useEffect(() => {
  ...
  const handleSave = useCallback(async () => {
    if (!localOcrRef.current || !resolvedOcrUrl) return;
    setIsSaving(true);
    setSaveError(null);
    try {
      const res = await fetch(`${BASE_URL}/admin/ocr/draft`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ocr_url: resolvedOcrUrl,
          ocr_data: localOcrRef.current,
        }),
      });
      if (!res.ok) {
      ...
      if (onSave && localOcrRef.current) {
        onSave({ file_path: filePath, ocr_data: localOcrRef.current });
      }
      onClose();
    } catch (e) {
  ...
  }, [resolvedOcrUrl, onSave, onClose, filePath]);
  ...
  const activeIme = LANG_IME_MAP[resolvedLang];
  ...
  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div
        ...
        style={{ width: "90vw", maxWidth: 1200, height: "90vh", display: "flex", flexDirection: "column" }}
      >
        {/* ── Modal Header ── */}
        <div className={styles.modalHeader}>
          <div className={styles.modalHeaderTitleWrap}>
            <i className="bi bi-pencil-square" style={{ color: "#4e8ef7", fontSize: "1.2rem" }} />
            <h3>Edit Document</h3>
            <span className={styles.modalIdBadge}>#{documentId}</span>
            {resolvedLang && (
              <span style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
            <button onClick={() => handleZoom("out")} disabled={zoom <= MIN_ZOOM} title="Zoom out" style={{
              display: "inline-flex", alignItems: "center", justifyContent: "center",
              width: 28, height: 28, borderRadius: 7, cursor: zoom <= MIN_ZOOM ? "not-allowed" : "pointer",
              border: "1.5px solid #e8dfc8", background: "#faf7f0", color: "#3A2C08",
              fontSize: 14, flexShrink: 0, opacity: zoom <= MIN_ZOOM ? 0.4 : 1,
            }}>
            ...
            <button onClick={() => handleZoom("in")} disabled={zoom >= MAX_ZOOM} title="Zoom in" style={{
              display: "inline-flex", alignItems: "center", justifyContent: "center",
              width: 28, height: 28, borderRadius: 7, cursor: zoom >= MAX_ZOOM ? "not-allowed" : "pointer",
              border: "1.5px solid #e8dfc8", background: "#faf7f0", color: "#3A2C08",
              fontSize: 14, flexShrink: 0, opacity: zoom >= MAX_ZOOM ? 0.4 : 1,
            }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    border: "1.5px solid #e8dfc8", background: "#faf7f0", color: "#3A2C08",
                    fontSize: 14, flexShrink: 0, opacity: currentPage <= 1 ? 0.4 : 1,
                  }}
                >
                  <i className="bi bi-chevron-left" />
                </button>
                <span style={{
                  fontSize: 11, fontWeight: 700, fontFamily: "monospace", color: "#3A2C08",
                  minWidth: 54, textAlign: "center", padding: "3px 6px", borderRadius: 5,
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "1.5px solid #e8dfc8", background: "#faf7f0", color: "#3A2C08",
                    fontSize: 14, flexShrink: 0, opacity: currentPage >= totalPages ? 0.4 : 1,
                  }}
                >
        ...
        <div
          ref={wrapperRef}
          style={{ flex: 1, background: "#f8fafc", overflow: "auto", position: "relative", minHeight: 0 }}
        >
          <div ref={containerRef} style={{ width: "100%", position: "relative" }}>
            {loading && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 40, color: "#666" }}>
                <p>Loading OCR viewer…</p>
              </div>
            )}
            {!loading && !resolvedOcrUrl && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 40, color: "#999" }}>
                <p>No OCR URL provided.</p>
              </div>
            )}
            {error && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 40, color: "#ef4444" }}>
                ⚠ {error}
              </div>
            )}
          </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  {activeIme && (
                    ...
                    >
                      ...
                      {resolvedLang} · {translitEnabled ? "ON" : "OFF"}
                    </button>
                  )}
                ...
                <div style={{ padding: "8px 10px 6px" }}>
                  <textarea
                    ...
                    placeholder={
                      activeIme && translitEnabled
                        ? `Type Roman phonetics for ${resolvedLang}…`
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: "#22c55e", cursor: "pointer", fontSize: 14, lineHeight: 1,
                        }}
                      >✓</button>
                    </div>
                  </div>
        ...
        <div className={styles.modalFooter} style={{ flexShrink: 0 }}>
          {saveError && (
            <span style={{ fontSize: 12, color: "#ef4444", marginRight: "auto", fontFamily: "monospace" }}>
              ⚠ {saveError}
            </span>
          )}
